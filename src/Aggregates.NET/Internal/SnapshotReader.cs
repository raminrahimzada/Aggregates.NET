...
namespace Aggregates.Internal
{
    ...
    class SnapshotReader : ISnapshotReader, IEventSubscriber
    {
        ...
        private Task onEvent(string stream, long position, IFullEvent e)
        {
            ...
            Logger.DebugEvent("GotSnapshot", "[{Stream:l}] bucket [{Bucket:l}] entity [{EntityType:l}] version {Version} {@Snapshot}", snapshot.StreamId, snapshot.Bucket, snapshot.EntityType, snapshot.Version, snapshot);
        ...
        public Task<ISnapshot> Retreive(string stream)
        {
            ...
            Snapshots.TryUpdate(stream, new Tuple<DateTime, string>(DateTime.UtcNow, snapshot.Item2), snapshot);

            ISnapshot deserialized = _serializer.Deserialize<Snapshot>(snapshot.Item2.AsByteArray());
