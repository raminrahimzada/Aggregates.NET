...
namespace Aggregates.Internal
{
    class EventStoreDelayed : IDelayedChannel
    {
        ...
        private static readonly App.Metrics.Core.Options.HistogramOptions MemCacheSize =
            new App.Metrics.Core.Options.HistogramOptions
            {
                Name = "Delayed Cache Size",
        ...
        static async Task Flush(object state, bool all = false)
        {
            ...
            flushState.Metrics.Measure.Histogram.Update(MemCacheSize, memCacheTotalSize);
            Logger.Write(LogLevel.Info,
                () =>
                    $"Flushing expired delayed channels - cache size: {memCacheTotalSize} - total channels: {MemCache.Keys.Count}");

            var totalFlushed = 0;

            // A list of channels who have expired or have more than 1/5 the max total cache size
            var expiredSpecificChannels =
                MemCache.Where(x => all || (DateTime.UtcNow - x.Value.Item1) > flushState.Expire ||
                                    (x.Value.Item3.Count > (flushState.MaxSize / 5)))
            ...
            await expiredSpecificChannels.WhenAllAsync(async (expired) =>
            {
                ...
                catch (Exception e)
                {
                    Logger.Write(LogLevel.Warn,
                        ...
                        MemCache.AddOrUpdate(expired,
                                (key) =>
                                    new Tuple<DateTime, SemaphoreSlim, List<IDelayedMessage>>(DateTime.UtcNow, fromCache.Item2, overLimit),
                                (key, existing) =>
                                {
